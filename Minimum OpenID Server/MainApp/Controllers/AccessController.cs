...
namespace App.Controllers
{
    public class AccessControl : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<IActionResult> RegisterUser([FromBody] UserRegisterData _userLoginInfo)
        {
            var _user = new IdentityUser
            {
                Email = _userLoginInfo.Login,
                LockoutEnabled = true,
                PasswordHash = "MySecretPasswordHash",
                AccessFailedCount = 3,
                UserName = _userLoginInfo.Login
            };

            var result = await _userManager.CreateAsync(_user, _userLoginInfo.Password);
